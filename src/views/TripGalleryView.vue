<template>
    <section
        ...
        <section class="h-full w-full flex justify-center relative">
            ...
            <teleport to="body">
                <div class="fixed top-0 left-0 backdrop-blur" v-if="selectedImage" @click="selectedImage = ''">
                    <img class="w-screen h-screen object-contain" :src="selectedImage" alt="">
                    <div class="w-full h-1/12 flex justify-center fixed bottom-16"><label for="fileUploader"
                            class="w-32 h-12 bg-secondary flex justify-center items-center text-white gap-2 rounded-full relative"><input
                                id="fileUploader" class="opacity-0 absolute w-full h-full rounded-full" @change="uploadFile"
                                type="file" multiple accept=".heic,  image/heic, image/*, video/*" />
